...
A few **representative examples** below (not all 14) show the **original figure
from the paper (left)** next to the **reproduction (right)**. They cover the main
threads — overall growth, material production, launch mass / closure, and a
parameter-sensitivity study — plus the one known divergence. The remaining
figures are in [`figures/`](figures/) and summarized in the
[fidelity table](#results--fidelity) above. Original panels are cropped from the
preprint PDF; reproductions come from [`figures.py`](figures.py).

**Growth of the industry** — exponential rise from ~8 MT to ~30,000 MT:

![Fig. 2 comparison](figures/comparisons/compare_fig02.png)

**Material production** — solids/fluids climb together; electronics emerge at Gen 3:

![Fig. 3 comparison](figures/comparisons/compare_fig03.png)

**Launch mass & the need for closure** — baseline stays low; skipping local
electronics/chip fabrication makes Earth-launched mass explode:

![Fig. 4 comparison](figures/comparisons/compare_fig04.png)

**Printer count vs reproduction time** — hyperbolic families per printer speed:

![Fig. 8 comparison](figures/comparisons/compare_fig08.png)

**Sensitivity to robonaut productivity (RWPA)** — slower robonauts need more
units early, all converging by Gen 6:

![Fig. 12 comparison](figures/comparisons/compare_fig12.png)

**⚠️ Known divergence — Gen-6 mass vs robonauts:** the paper shows a gentle
inverted-U peaking at ~0.2–0.25 kg/hr; the reproduction is monotonic. The axis
magnitudes and the primary trend match, but the peak does not (see
[Known divergences](#known-divergences)):

![Fig. 11 comparison](figures/comparisons/compare_fig11.png)

> These composites are built by [`make_comparisons.py`](make_comparisons.py),
> which renders the relevant PDF pages, crops the originals, and pairs them with
> the reproductions.
