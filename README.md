...
# (optional) rebuild the original-vs-reproduction comparison images
.venv/bin/pip install pymupdf pillow
.venv/bin/python make_comparisons.py
```
...
Each pair below shows the **original figure from the paper (left)** next to the
**reproduction (right)**. Original panels are cropped from the preprint PDF in
[`docs/`](docs/); reproductions are generated by [`figures.py`](figures.py).
Full-resolution composites live in [`figures/comparisons/`](figures/comparisons/).

### Fig. 2 — Growth of lunar industry (mass & robonauts)
![Fig. 2 comparison](figures/comparisons/compare_fig02.png)

### Fig. 3 — Production of materials & parts
![Fig. 3 comparison](figures/comparisons/compare_fig03.png)

### Fig. 4 — Mass launched to Moon (closure-failure variants)
![Fig. 4 comparison](figures/comparisons/compare_fig04.png)

### Fig. 5 — Power available vs needed
![Fig. 5 comparison](figures/comparisons/compare_fig05.png)

### Fig. 6 — Asset mass by duty cycle
![Fig. 6 comparison](figures/comparisons/compare_fig06.png)

### Fig. 7 — Robonauts by duty cycle
![Fig. 7 comparison](figures/comparisons/compare_fig07.png)

### Fig. 8 — Printers per set vs reproduction time
![Fig. 8 comparison](figures/comparisons/compare_fig08.png)

### Fig. 9 — Asset mass by year (constant production per generation)
![Fig. 9 comparison](figures/comparisons/compare_fig09.png)

### Fig. 10 — Robonauts by year
![Fig. 10 comparison](figures/comparisons/compare_fig10.png)

### Fig. 11 — Gen-6 mass vs robonauts  ⚠️ *known divergence*
The paper shows a gentle inverted-U peaking at ~0.2–0.25 kg/hr; the reproduction
is monotonic. Axis magnitudes and the primary trend match, but the peak does
not. See [Known divergences](#known-divergences).
![Fig. 11 comparison](figures/comparisons/compare_fig11.png)

### Fig. 12 — Robonauts by robonaut productivity (RWPA)
![Fig. 12 comparison](figures/comparisons/compare_fig12.png)

### Fig. 13 — Asset mass by robonaut productivity (RWPA)
![Fig. 13 comparison](figures/comparisons/compare_fig13.png)

### Fig. 14 — Launch mass by robonaut productivity (RWPA)
![Fig. 14 comparison](figures/comparisons/compare_fig14.png)

> Regenerate the originals and composites with the helper script
> [`make_comparisons.py`](make_comparisons.py).
